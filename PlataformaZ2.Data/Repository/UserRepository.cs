...
namespace PlataformaZ2.Data.Repository
{
    ...
    public class UserRepository : BaseRepository<UserDao>
    {
        ...
        /// <returns>boolean result</returns>
        public bool CheckCpf(int? idUser, string cpf)
        {
            try
            {
                var query = GetSession().Query<UserDao>();
                ...
                if (idUser.HasValue)
                {
                    result = query.Any(x => x.Cpf.Equals(cpf) && x.Id != idUser && x.Active);
                }
                else
                {
                    result = query.Any(x => x.Cpf.Equals(cpf) && x.Active);
                }
        ...
        public PaginationResponseFromRepository<UserDao> GetByFilter(UserFilterDto filter)
        {
            ...
            {                
                ...
                {
                    query = query.Where(x => x.Name.Contains(filter.NameOrUsername) || x.Username.Contains(filter.NameOrUsername));
                }
                
                if (filter.IdProfile.HasValue)
                {
                    query = query.Where(x => x.Profile.Id.Value == filter.IdProfile.Value);
                }
